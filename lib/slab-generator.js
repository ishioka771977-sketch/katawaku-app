...
(function (root, factory) {
  ...
  const SCOPE_LABEL = { steel_existing: '鋼製型枠（上部工で施工済）— コンパネ不要', none: '型枠不要' };
  function makeSideFace(id, name, width_mm, height_mm, thickness_mm, finish, scope) {
    if (scope && scope !== 'plywood') {
      return { id, name, face_type: 'side', width_mm, height_mm, finish, layout_method: null, existing_form: scope,
        panels: [], separators: null, note: SCOPE_LABEL[scope] || scope };
    }
    return {
      id,
      name,
  ...
  function summarizePanels(faces) {
    ...
    for (const f of faces) {
      if (f.face_type === 'haunch') {
        if (f.total_panels > 0) summary.push({ face: 'ハンチ', size: '加工材', type: 'カット', count: f.total_panels, area_m2: null });
        continue;
      }
      if (!f.panels || !f.panels.length) continue;
  ...
  function generateSlabJson(p) {
    ...
    const hd = p.haunch_depth_mm == null ? 50 : p.haunch_depth_mm;   // null=不明→標準50、0=ハンチなし
    const hw = p.haunch_width_mm == null ? 200 : p.haunch_width_mm;
    const hasHaunch = hd > 0 && hw > 0;
    const scope = p.formwork_scope || {};
    const endScope = scope.end_forms || 'plywood';
    const sideScope = scope.side_forms || 'plywood';
    ...
    const faces = [
      makeSideFace('A', `橋軸方向端部（A1側・妻型枠）${skewTag}`, endW, H, T, '埋設（地覆で隠れる）', endScope),
      makeSideFace("A'", `橋軸方向端部（A2側・妻型枠）${skewTag}`, endW, H, T, '埋設（地覆で隠れる）', endScope),
      makeSideFace('B', '橋幅方向端部（上流側）', L, H, T, '埋設', sideScope),
      makeSideFace("B'", '橋幅方向端部（下流側）', L, H, T, '埋設', sideScope),
      {
        id: 'H',
        name: 'ハンチ部（主桁取合い）',
        face_type: 'haunch',
        note: hasHaunch ? '主桁上フランジとの取合い。角度カット合板' : 'ハンチなし（設計書・代理人確認）',
        haunch_depth_mm: hasHaunch ? hd : 0,
        haunch_width_mm: hasHaunch ? hw : 0,
        girder_count: gc,
        panels_per_girder: hasHaunch ? 2 : 0,
        total_panels: hasHaunch ? gc * 2 : 0,
        panel_note: hasHaunch ? '三角形にカットした合板。主桁1本につき左右2枚' : 'ハンチ型枠なし',
      },
    ];
    ...
    const notes = [
      ...
      { category: 'ハンチ', content: hasHaunch ? '主桁上フランジとの取合い部。角度カット合板で傾斜面を形成。隙間はテープで塞ぐ' : 'ハンチなし。ハンチ型枠は不要' },
    ...
    if (endScope !== 'plywood' || sideScope !== 'plywood') {
      notes.push({ category: '型枠範囲', content: `妻型枠（A/A'）: ${SCOPE_LABEL[endScope] || 'コンパネ割付'}／側型枠（B/B'）: ${SCOPE_LABEL[sideScope] || 'コンパネ割付'}。コンパネ・セパ数量は割付した面のみ計上` });
    }
    if (isSkew) {
    ...
    return {
      ...
      structure: {
        ...
        formwork_config: {
          ...
          side_form_required: sideScope === 'plywood' || endScope === 'plywood',
          end_form_scope: endScope,
          side_form_scope: sideScope,
          haunch_form_required: hasHaunch,
      ...
      quantities: {
        panels: {
          ...
          note: totalCount ? `全てカット材（${H}mm幅にカット）。コンパネ原板(900×1800)換算: 約${rawSheets}枚` : 'コンパネ型枠なし（鋼製型枠施工済み等）',
        },
        ...
        misc: [
          // コンパネ型枠が無い（鋼製型枠施工済み等）なら桟木・面木・剥離剤・テープは0
          { name: '桟木', spec: '30×60×3600', count: totalCount ? Math.max(30, Math.ceil((2 * (endW + L)) / 3600 / 2)) : 0, unit: '本' },
          { name: '面木', spec: '15×15三角', count: totalCount ? Math.max(60, Math.ceil((2 * (endW + L)) / 1000)) : 0, unit: 'm' },
          { name: '剥離剤', spec: '鉱物油系', count: totalCount ? 1 : 0, unit: '缶' },
          { name: '布テープ', spec: '50mm幅', count: totalCount ? 5 : 0, unit: '巻' },
  ...
  function checkSlabJson(data) {
    ...
    for (const ph of data.phases || []) {
      for (const f of ph.faces || []) {
        ...
        if (!f.panels || !f.panels.length) continue; // 鋼製型枠施工済み等＝割付なし
