...
## 追記（実装＆実機検証後の訂正） 2026-06-27

実装と実機検証の結果、当初想定の一部を訂正する。

**実施済み（検証済み・main マージ済み）**
- エンジン四隅化（`createFaceMesh`＋`buildQuadGeometry`/`quadTaper`/`quadTopSlope`）
- 擁壁: 妻面台形＋前面傾斜
- 橋台: ウイング壁の縦断台形＋竪壁の対称テーパ（リフト対応）
- 床版: 2D断面にむくり（camber）反映（床版は build3D 無し＝2Dのみ）

**訂正: parapet / box_culvert のハンチは「三角形化が必要」ではなかった**
- 地覆/壁高欄・BOXのハンチは**延長方向に一様な線状の面取り**であり、
  `createFaceMesh(face, L, slant)` を実角度 `atan2(H,W)` で傾けた**長方形バンド**＝
  既に幾何学的に正しい（box は ExtrudeGeometry で押し出し＝同じく正しい）。
- 三角形ジオメトリが要るのは「長さ方向に変化するハンチ（隅角でテーパ等）」のみ。
  一様な線状ハンチを"正規化"すると正しい描画を壊すため、**変更しない**。

**残（別タスク）**
- 床版の3D確認ビュー新規構築（現状 build3D 無し）
- 長さ方向に変化する非一様ハンチ・他構造物との取り合い（マルチ構造シーン）
